...
export function countTimedOut(resultsArr) {
  ...
  export function deriveCountsFromResults(resultsArr) {
    ...
    for (const q of arr) {
      ...
      if (type === "live") {
        ...
        if (isTimed) {
          timedOut++;
          // timed out is NOT "skipped"
          wrong++; // because it wasn't correct and the user effectively failed it
        } else if (!hasAttempt) {
          skipped++;
        } else if (q?.isCorrect) {
          correct++;
  ...
  export function getDisplayBreakdown(resultLike) {
    ...
    const hasResults = Array.isArray(resultLike?.results) && resultLike.results.length > 0;
